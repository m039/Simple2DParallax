...
namespace m039.Parallax
{
	...
	public class ComplexParallaxLayer : BaseParallaxLayer
	{
		...
		void Regenerate()
		{
			...
			SpriteRenderer createSprite(int xIndex, int yIndex)
			{
			...
			{
				var spriteCenter = spriteRenderer.bounds.center;
			...
			bool createRow(int yIndex, bool excludeCenter)
			{
				...
				while (true)
				{
					var spriteRenderer = createSprite(x--, yIndex);
				...
				if (!excludeCenter)
				{
					var spriteRenderer = createSprite(0, yIndex);
